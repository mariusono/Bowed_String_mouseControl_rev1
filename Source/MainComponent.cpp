...
//    activeStrings.resize(polyphony, nullptr);
    ...
    // Timer Callback for force extract
...
//        Logger::getCurrentLogger()->outputDebugString("Mouse dragged!");
        ...
        x_inp_var = gPosition[1];

        x_inp_var = clamp(x_inp_var, 0.1, 0.9);

        gPositionRaw[0] = linearMapping(0.0, 1.0, 0.0, widthScreen, gPosition[0]);
        gPositionRaw[1] = linearMapping(0.0, 1.0, 0.0, heightScreen, gPosition[1]);
...
//        Logger::getCurrentLogger()->outputDebugString("Vb Raw: (" + String(VbRaw) + ")");
        ...
        vB_var = Vb; // IMPORTANT! this is the most important part..
        ...
        gPositionPrev[0] = gPosition[0]; // also damn important !
...
void MainComponent::mouseUp(const MouseEvent &e)
{
    ...
    isBowing = false; // this does nothing ! calculations are still happening..
    fN_var = 0;
    vB_var = 0;
...
//    double maxFB = 1.0;
    double maxFB = 0.5;
    FB_wheel = linearMapping(-5.0, 0.0, maxFB, 0.0, wheelDeltaY);
    Logger::getCurrentLogger()->outputDebugString("FB_wheel: (" + String(FB_wheel) + ")");

    
    opa_level = FB_wheel/maxFB;
    if (opa_level < 0.1)
    {
        opa_level = 0.1;
    }
